...
int user_options_getopt (hashcat_ctx_t *hashcat_ctx, int argc, char **argv)
{
  ...
  // JSON is the deterministic winner if both reporting forms were requested,
  // regardless of their order on the command line.

  pipe_profile_configure (user_options->stage_profile || user_options->stage_profile_json, user_options->stage_profile_json);
...
static void user_options_alias_attack_mode (hashcat_ctx_t *hashcat_ctx)
{
  ...
  // Shooter applies -r/-g after the complete candidate in these modes. Keep those attacks on their
  // original host-built paths; the upstream -a 12 aliases use the combinator amplifier for the mask.

  if ((user_options->rp_files_cnt > 0) || (user_options->rp_gen > 0)) return;
...
void user_options_extra_init_late (hashcat_ctx_t *hashcat_ctx)
{
  ...
  if (user_options->attack_mode == ATTACK_MODE_HYBRID2)
  {
    ...
    if ((user_options_extra->whole_candidate_rules == false) && (optimized_kernel == false)) return;

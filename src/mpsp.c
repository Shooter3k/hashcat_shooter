...
int mask_ctx_update_loop (hashcat_ctx_t *hashcat_ctx)
{
  ...
  {
    // Shooter's unaliased -a 1 path is entirely wordlist-backed, including
    // the three-or-more-file extension. It has no mask context to update.
    ...
    else if (user_options_extra->base_source == BASE_SOURCE_MASK)
    {
      mask_ctx->mask = mask_ctx->masks[mask_ctx->masks_pos];
...
int mask_ctx_init (hashcat_ctx_t *hashcat_ctx)
{
        ...
        || (user_options->attack_mode == ATTACK_MODE_MULTI_HYBRID))
  {
    ...
    char *arg = user_options_extra->hc_workv[(user_options->attack_mode == ATTACK_MODE_HYBRID1) ? (user_options_extra->hc_workc - 1) : 0];

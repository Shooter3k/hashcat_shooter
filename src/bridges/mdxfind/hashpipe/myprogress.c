/*
 * myprogress.c — Progress OpenEdge ENCODE function (sliced-by-8 implementation)
 *
 * This is a performance-optimized implementation of the proprietary password
 * hashing function from Progress Software Corporation's OpenEdge 4GL/ABL
 * database system.  Progress never published the algorithm; it was reverse-
 * engineered and released under CC0 by Pieter van Ginkel:
 *   https://github.com/pvginkel/ProgressEncode
 *
 * Algorithm:
 *   - Core primitive: CRC-16/USB (polynomial 0x8005, reflected).
 *   - 5 rounds: XOR-fold input into a 16-byte scratch buffer, then
 *     iteratively apply CRC-16 to produce 8 x 16-bit values that become
 *     the new scratch state.
 *   - Output: each result byte is mapped to a printable alpha character —
 *     (byte & 0x7F) kept if A-Z or a-z, else (byte >> 4) + 0x61 (-> 'a'-'p').
 *     dest_lookup[] is a pre-computed table of this mapping.
 *   - Produces a 16-character alphabetic string.
 *
 * The reference implementation processes the 16-byte scratch one byte at a
 * time through a single 256-entry CRC-16 table.  This version uses 8 sliced
 * CRC tables (crc_table[8][256]) to process 8 bytes per step via two 64-bit
 * accumulators (sc1, sc2).
 *
 * This implementation: October 2017 (RCS rev 1.1, 2017-10-28).
 * Hashcat mode 26200: added v6.2.2, June 2021.
 * pvginkel reference implementation: https://github.com/pvginkel/ProgressEncode
 *   (first commit 2014-06-15, byte-at-a-time; this sliced-by-8 version is
...
const uint16_t crc_table[8][256] = {
  {
...
const char dest_lookup[256] = 
  {
    ...
    0x65, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 
    0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x66, 0x66, 0x66, 0x66, 0x66, 
    0x67, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 
    0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x68, 0x68, 0x68, 0x68, 0x68, 
    ...
    0x6d, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 
    0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x6e, 0x6e, 0x6e, 0x6e, 0x6e, 
    0x6f, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 
    0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x70, 0x70, 0x70, 0x70, 0x70
  };
...
{
  uint16_t hash = 0x0cc0;
  int i,j;
  uint64_t sc1,sc2,tmp, *ptr;
  ...
  for (i = 0; i < 5; i++)
  {
    ...
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
           crc_table[3][(tmp >> 24) & 0xff] ^
           crc_table[4][(tmp >> 32) & 0xff] ^
           crc_table[5][(tmp >> 40) & 0xff] ^
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    /*  For whatever reason after the first 8 bytes
     *  the checksum has to be xored with 0x9000.
     */ 
    if (i == 0) {
      tmp = sc1 ^ bswap64( (uint64_t)(hash ^ 0x9000));
    }
    else {
      tmp = sc1 ^ bswap64( (uint64_t)(hash));
    }
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc1 = (sc1 & 0xffffffffffff0000L) | hash;
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc1 = (sc1 & 0xffffffff0000ffffL) | ((uint64_t)hash << 16);
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc1 = (sc1 & 0xffff0000ffffffffL) | ((uint64_t)hash << 32);
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc1 = (sc1 & 0x0000ffffffffffffL) | ((uint64_t)hash << 48);
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc2 = (sc2 & 0xffffffffffff0000L) | hash;
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc2 = (sc2 & 0xffffffff0000ffffL) | ((uint64_t)hash << 16);
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc2 = (sc2 & 0xffff0000ffffffffL) | ((uint64_t)hash << 32);
    tmp = sc2 ^ bswap64( (uint64_t)hash);
    hash = crc_table[0][ tmp        & 0xff] ^
           crc_table[1][(tmp >>  8) & 0xff] ^
           crc_table[2][(tmp >> 16) & 0xff] ^
    ...
    hash = crc_table[0][ tmp        & 0xff] ^
           ...
           crc_table[6][(tmp >> 48) & 0xff] ^
           crc_table[7][(tmp >> 56) & 0xff];
    sc2 = (sc2 & 0x0000ffffffffffffL) | ((uint64_t)hash << 48);
  }

  dest[ 0] = dest_lookup[sc1 & 0xff];
  dest[ 1] = dest_lookup[(sc1 >> 8) & 0xff];
  dest[ 2] = dest_lookup[(sc1 >>16) & 0xff];
  dest[ 3] = dest_lookup[(sc1 >>24) & 0xff];
  dest[ 4] = dest_lookup[(sc1 >>32) & 0xff];
  dest[ 5] = dest_lookup[(sc1 >>40) & 0xff];
  dest[ 6] = dest_lookup[(sc1 >>48) & 0xff];
  dest[ 7] = dest_lookup[(sc1 >>56) & 0xff];
  dest[ 8] = dest_lookup[sc2 & 0xff];
  dest[ 9] = dest_lookup[(sc2 >> 8) & 0xff];
  dest[10] = dest_lookup[(sc2 >>16) & 0xff];
  dest[11] = dest_lookup[(sc2 >>24) & 0xff];
  dest[12] = dest_lookup[(sc2 >>32) & 0xff];
  dest[13] = dest_lookup[(sc2 >>40) & 0xff];
  dest[14] = dest_lookup[(sc2 >>48) & 0xff];
  dest[15] = dest_lookup[(sc2 >>56) & 0xff];
}

...
The modification depends on the attack mode. Hashcat supports the core attack modes 0, 1, 3, 6, 7 and 12, while Shooter also adds attack mode 13. Attack modes 6, 7 and 12 share attack mode 1 kernel code. Shooter attack mode 13 executes its ordered wordlist, mask and rule stages on the host and sends each finished candidate through the attack mode 0 straight kernel with a no-op GPU rule. A hash module therefore still implements the same three kernel families in source files 0, 1 and 3.
